...
class Model:
    def __init__(self, train = False):
        df = Model_utils.load_data('std_df.csv')
        ...
        if train == True:
            print("retraining")
            self.models = [
                Model_utils.train_and_save(w[0], w[2], f'models/model_year_{i}.pkl')
                for i, w in enumerate(self.windows)
            ]
        else:
            print("loading trained model")
            self.models = [
                joblib.load(f'models/model_year_{i}.pkl')
    ...
    def graph_feature(self, feature_name):
        ...
        plt.tight_layout()
        plt.show()
        plt.savefig(f'Coefficient_Evolution_{feature_name}.png')  # Save the plot with a dynamic filename
        plt.close()
