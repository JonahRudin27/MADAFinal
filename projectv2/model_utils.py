...
class Model_utils:
    ...
    def preprocess(
        ...
        X_raw = df.drop(columns=['COVID Deaths'])  # still a DataFrame
        ...
        for i in range(len(unique_times) - window_size + 1):  # still increments by 1 month
            ...
            window_df = df[(df['Year'] >= start_time) & (df['Year'] <= end_time)]
            X_window = window_df.drop(columns=['COVID Deaths'])
            y_window = window_df['COVID Deaths']
    ...
    def predict_new(
        ...
        if 'COVID Deaths' in df_new.columns:
            df_new = df_new.drop('COVID Deaths', axis=1)
